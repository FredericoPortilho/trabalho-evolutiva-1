...
#ifndef EVOLUTIONARY_COMPUTATION_ISLAND_EA_H
#define EVOLUTIONARY_COMPUTATION_ISLAND_EA_H
...
public:
    ...
    public:
        ...
        void mutation(problem &p, double mutation_strength) { s.mutation(p,mutation_strength); };

        individual crossover(problem &p, individual &rhs) { return s.crossover(p,rhs.s); };

        double distance(problem &p, individual &rhs, double max_dist = std::numeric_limits<double>::max()) { return this->s.distance(p,rhs.s,max_dist); };
...
public:
    ...
    using population_type = std::vector<std::shared_ptr<individual>>;
...
private /* members */:
    ...
    size_t _migration_epoch = 5;
    size_t _migration_size = 50;
    double _fitness_sharing_niche_size = 50;
    // Stopping criteria
    ...
    size_t _number_of_best_solutions = 5;

};

#endif //EVOLUTIONARY_COMPUTATION_ISLAND_EA_H
